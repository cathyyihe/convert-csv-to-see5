...
class Arff():
    ...
    def generate_arff(self):
        ...
        # Show Attributes
        for i in range(0,totalCols):
            # print("@attribute" + " " + attributes[i] + " " + attTypes[i])
        ...
        for j in range(1,totalRows):
            for i in range(totalCols):
                if " " in allData[j][i]:
                    allData[j][i]=str("'"+allData[j][i]+"'")
                    # print(allData[j][i])
            # print(','.join(allData[j]))
            writeFile.write(','.join(allData[j])+"\n")
        writeFile.close()

        # print(self.fileToRead + " was converted to " + self.fileToWrite)

# Arff("2mergedcsv13.csv","2mergedcsv13.arff").generate_arff()
# Arff("1anatesting.csv","1anatesting.arff").generate_arff()
# Arff("csv4.csv","csv4.arff").generate_arff()
